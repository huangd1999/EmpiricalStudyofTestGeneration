...
def test_report(dataset, lg,task,model):
    ...
    for item in tqdm(dataset):
        tests = [test.strip() for test in item["incorrect_tests"].split("/n") if test.strip()]
        ...
        if result["passed"] and len([line for line in item["correct_tests"].split("\n") if "assert" in line and "assert function" not in line])>0:
            ...
            with open(temp_file_name,"w") as temp_file:
                temp_file.write(full_code)
            stmts = count_code_lines(temp_file_name)

            coverage_results["canonical_solution_test_case"][item["task_id"]] = {"stmts": stmts, "miss": stmts, "cover": 0,"pass":result["passed"]}
...
def fetch_completion(task,model):
    for prompt in ["correct","incorrect"]:
        ...
        for i in range(len(dataset)):
            ...
            dataset[i]["canonical_solution"] = dataset[i]["correct"]
        result = test_report(dataset,"python",task,model)

        with open(f"../source_codes/rq12_22_pyter_{model}_{prompt}.json","w") as f:
